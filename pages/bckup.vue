<script setup>
const { page, title, slug, pageTitle, pageType, meta, day, auth, setItem } = useCommon();
...
//   if (!auth.value) {
//     // message.value = 'First Login intot the Application'
//   } else {
//     leadsFormData.value.name = auth.value.name;
//     // console.log(auth.value.name)
...
const submitQueryForm = async (formData) => {
  console.log(formData)

  // formData.value.subcategory = data.split("-in-")[0]
  // formData.value.city = city
  // formData.value.query = data
  ...
  <template v-else-if="pageType == 'Subcategories' && data != 'profile'">
    <section>
      ...
      <div class="columns is-multiline is-mobile is-variable is-2-tablet mt-4 mb-6">
        ...
        >
          <a
            :href="`/${slug(city)}/${slug(s?.name)}-in-${slug(city)}`"
            class="grid-item box"
          >
            <p>
              <b>{{ s.name }}</b>
      ...
      <span class="is-left">
        <nav class="breadcrumb" aria-label="breadcrumbs">
          <ul>
            ...
            <li>
              <nuxt-link to="/">{{ title(city) }} </nuxt-link>
            </li>
            <li>
              <nuxt-link :to="data"
                >{{ meta.page_title }} in {{ title(city) }}
              </nuxt-link>
            </li>
            <li class="is-active">
              <a href="javascript:;" aria-current="page">{{ contents.length }}</a>
            </li>
          </ul>
        </nav>
        <h1 class="title is-1">{{ meta.page_title }} in {{ title(city) }}</h1>
        <p class="mt-4">
          {{ meta.page_content.replace(new RegExp("cityName", "g"), title(city)) }}
        </p>
      </span>

      <div class="blog-section mt-6 mb-6">
        <div class="blog-content">
          <div v-for="(b, index) in contents" :key="index">
            <nuxt-link
              class="media mt-4"
            ...
            >
              <div class="media-left">
                <figure class="image custom-image">
                  <img
                    :src="`${b?.business_images[0] ?? '../Image_not_available.png'}`"
                    alt=" Blog Image"
                  />
                </figure>
              </div>
              <div class="media-content">
                ...
                <div>
                  <i class="fa fa-phone" aria-hidden="true"></i> {{ b.business_phone }}
                </div>

                <div v-if="b.business_services.length > 0" class="mt-2">
                  <div class="buttons">
                    <button
                      class="button is-primary is-ligh is-small"
                      v-for="service in b.business_services"
                      :key="service"
                    >
                      {{ service }}
                    </button>
        ...
        <aside class="sidebar">
          <div class="form-box">
            <Formkit type="form" @submit="submitQueryForm" submit-lebel="Query Form" id="queryForm">
  ...
  <template v-else-if="pageType == 'Business Details' && data != 'profile'">
    <section>
      <span class="is-left">
        <nav class="breadcrumb" aria-label="breadcrumbs">
          <ul>
            ...
            <li>
              <nuxt-link :to="data">{{ contents?.business_name }} </nuxt-link>
            </li>
          </ul>
        </nav>
      ...
      <div class="columns mt-4" v-if="images.length > 0">
        ...
        <div class="column">
          <div class="content-section">
            <div class="custom-container">
              ...
              <p>
                {{ contents?.business_address }}, {{ contents?.business_locality }},
                {{ contents?.business_city }},{{ contents?.business_state }},{{
                  contents?.business_pin
                }}
              </p>
            </div>
            <div class="custom-container">
              ...
              <a
                :href="'tel: ' + contents?.business_phone"
                target="_blank"
                rel="noopener"
              >
        ...
        <Map
          :lat="contents?.business_latitude"
          :long="contents?.business_longitude"
        ...
        <div class="columns three-layout">
          <div class="column">
            <div class="custom-container">
              ...
              <a
                :href="'mailto: ' + contents?.business_email"
                target="_blank"
                rel="noopener"
              >
            ...
            <div class="custom-container">
              ...
              <p>
                <a
                  :href="contents?.business_social?.facebook"
                  target="_blank"
                  rel="noopener"
                >
              ...
              <p>
                <a
                  :href="contents?.business_social?.instagram"
                  target="_blank"
                  rel="noopener"
                >
              ...
              <p>
                <a
                  :href="contents?.business_social?.youtube"
                  target="_blank"
                  rel="noopener"
                >
