...
export default function useCommon() {
    ...
    const pageLimit = useState('pageLimit', () => 10)
    ...
    const setPage = (val) => {
        pageSlug.value = val
        useFetch(`/api/pages?slug=${val}`, { method: 'get'
    ...
    const setAction = (val) => { action.value = val; return; }
    ...
    const setModule = (val) => {
        ...
        if (val == 'businesses') {
            if (authDataStore.authData.role === 'Admin') { td.value = ['id', 'Name', 'Category', 'City', 'Content By', 'Approved By', 'Date', 'Status', 'Action'] }
        ...
        $fetch(`/api/modules?name=${module.value}&pageLimit=${pageLimit.value}`, { method: 'get' }).then((res) => { rows.value = res }); return;
    }
    return {
        encryptObject,
        decryptObject,
